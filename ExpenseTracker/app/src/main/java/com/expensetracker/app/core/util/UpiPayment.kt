...
fun parseUpiQr(rawValue: String): UpiPayee? {
     ...
     * QRs with no `am` are free-form payment requests: the payer must be
     * able to provide both the amount and note before launching their UPI app.
     */
    val isDynamic =
 ...
 * - Does NOT preserve unsupported/provider-specific `aid`.
 *
...
fun buildUpiPaymentUri(
     ...
     * Dynamic/signed QR:
     *
     * Never rebuild this URI. Transaction references and signatures may be
     * tied to the exact URI generated by the merchant.
     */
    if (payee.isDynamic && payee.originalUri != null) {
     ...
     * Static/P2P QR:
     *
     * Rebuild the URI instead of appending to originalUri.
     *
     * This intentionally removes unsupported/provider-specific parameters
     * such as `aid` from the final payment URI.
     */
    return Uri.Builder()
        .scheme("upi")
        .authority("pay")
        ...
        .apply {
            ...
            /*
             * Preserve only safe static parameters.
             *
             * `aid` is no longer included in KNOWN_UPI_EXTRA_PARAMS, so it
             * will not appear in payee.extraParams.
             */
            payee.extraParams.forEach { (key, value) ->
                if (key in SAFE_STATIC_UPI_PARAMS) {
                    appendQueryParameter(key, value)
                }
            }

            /*
             * UPI amount is in rupees, not paise.
             *
             * Example:
             * amount = "1"
             * produces am=1
             *
             * Do NOT pass amountMinor here because amountMinor is stored
             * internally in paise.
             */
            amount
                .trim()
