...
class ExpenseRepositoryImpl(
    ...
    override fun observeExpensesForMonth(profileId: Long, yearMonth: YearMonth): Flow<List<Expense>> =
        expenseDao.observeBetween(profileId, yearMonth.startEpochDay, yearMonth.endEpochDay).map { it.map(ExpenseEntity::toDomain) }

    override fun observeCategoryTotals(profileId: Long, yearMonth: YearMonth): Flow<Map<ExpenseCategory, Long>> =
        expenseDao.observeCategoryTotalsForBudget(profileId, yearMonth.toString()).map { totals -> totals.associate { it.category to it.totalMinor } }

    override fun observeMonthlyTotal(profileId: Long, yearMonth: YearMonth): Flow<Long> =
        observeCategoryTotals(profileId, yearMonth).map { it.values.sum() }

    override fun observeIncomeTotal(profileId: Long, yearMonth: YearMonth): Flow<Long> =
        expenseDao.observeIncomeTotalForBudget(profileId, yearMonth.toString())
    ...
    override suspend fun updateExpense(expense: Expense): AddExpenseResult = withContext(ioDispatcher) {
        ...
        val alerts = try { evaluateAndNotify(expense.profileId, expense.budgetMonth.ifBlank { YearMonth.from(expense.date).toString() }, expense.category) } catch (_: SQLiteException) { emptyList() }
    ...
    private suspend fun evaluateAndNotify(profileId: Long, budgetMonth: String, changedCategory: ExpenseCategory): List<LimitAlert> {
        ...
        budgetLimitDao.getByKey(profileId, changedCategory.name)?.let { limit ->
            val spent = expenseDao.getCategoryTotalForBudget(profileId, changedCategory, budgetMonth)
        ...
        budgetLimitDao.getByKey(profileId, OVERALL_BUDGET_KEY)?.let { limit ->
            val spent = expenseDao.getOverallTotalForBudget(profileId, budgetMonth)
