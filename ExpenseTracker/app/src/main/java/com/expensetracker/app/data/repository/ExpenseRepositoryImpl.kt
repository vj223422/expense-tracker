...
class ExpenseRepositoryImpl(
    ...
    override fun observeAllExpenses(profileId: Long): Flow<List<Expense>> = expenseDao.observeAll(profileId).map { entities -> entities.map { it.toDomain() } }

    override fun observeExpensesForMonth(profileId: Long, yearMonth: YearMonth): Flow<List<Expense>> =
        expenseDao.observeBetween(profileId, yearMonth.startEpochDay, yearMonth.endEpochDay).map { entities -> entities.map { it.toDomain() } }

    override fun observeCategoryTotals(profileId: Long, yearMonth: YearMonth): Flow<Map<ExpenseCategory, Long>> =
        expenseDao.observeCategoryTotals(profileId, yearMonth.startEpochDay, yearMonth.endEpochDay).map { totals -> totals.associate { it.category to it.totalMinor } }
    ...
    override fun observeIncomeTotal(profileId: Long, yearMonth: YearMonth): Flow<Long> =
        expenseDao.observeIncomeTotal(profileId, yearMonth.startEpochDay, yearMonth.endEpochDay)
    ...
    override suspend fun addExpense(profileId: Long, amountMinor: Long, category: ExpenseCategory, note: String, date: LocalDate, isIncome: Boolean): AddExpenseResult = withContext(ioDispatcher) {
        val expenseId = try {
            expenseDao.insert(ExpenseEntity(profileId = profileId, amountMinor = amountMinor, category = category, note = note, isIncome = isIncome, epochDay = date.toEpochDay(), createdAtEpochMillis = System.currentTimeMillis()))
        ...
        val alerts = try { evaluateAndNotify(profileId, YearMonth.from(date), category) } catch (e: SQLiteException) { emptyList() }
    ...
    override suspend fun updateExpense(expense: Expense): AddExpenseResult = withContext(ioDispatcher) {
        try { expenseDao.update(expense.toEntity()) } catch (e: SQLiteException) { return@withContext AddExpenseResult.Error("Couldn't update expense — local storage error.") }
        if (expense.isIncome) return@withContext AddExpenseResult.Success(emptyList())
        val alerts = try { evaluateAndNotify(expense.profileId, YearMonth.from(expense.date), expense.category) } catch (e: SQLiteException) { emptyList() }
    ...
    override suspend fun restoreExpense(expense: Expense): AddExpenseResult = withContext(ioDispatcher) {
        try { expenseDao.insert(expense.toEntity()) } catch (e: SQLiteException) { return@withContext AddExpenseResult.Error("Couldn't restore expense — local storage error.") }
        if (expense.isIncome) return@withContext AddExpenseResult.Success(emptyList())
        val alerts = try { evaluateAndNotify(expense.profileId, YearMonth.from(expense.date), expense.category) } catch (e: SQLiteException) { emptyList() }
        AddExpenseResult.Success(alerts)
    }
    ...
    private suspend fun evaluateAndNotify(profileId: Long, yearMonth: YearMonth, changedCategory: ExpenseCategory): List<LimitAlert> {
        ...
        budgetLimitDao.getByKey(profileId, changedCategory.name)?.let { limit ->
            ...
            maybeAlert(profileId, "${periodPrefix}_${changedCategory.name}", changedCategory, spent, limit.limitMinor)?.let(alerts::add)
        }
        budgetLimitDao.getByKey(profileId, OVERALL_BUDGET_KEY)?.let { limit ->
            ...
            maybeAlert(profileId, "${periodPrefix}_$OVERALL_BUDGET_KEY", null, spent, limit.limitMinor)?.let(alerts::add)
        }
        return alerts
    }
