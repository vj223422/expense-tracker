...
class NotificationHelper(private val context: Context) {
    init {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, context.getString(R.string.notification_channel_budget_alerts), NotificationManager.IMPORTANCE_DEFAULT).apply {
    ...
    fun notify(alert: LimitAlert) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(android.R.drawable.ic_dialog_info).setContentTitle(title).setContentText(body).setPriority(NotificationCompat.PRIORITY_DEFAULT).setAutoCancel(true).build()
    ...
    fun notifyIncomeAdded(amountMinor: Long, source: String, date: LocalDate, expenseId: Long, salaryDetected: Boolean) {
        ...
        if (salaryDetected) {
            ...
            val actionIntent = Intent(context, BudgetCycleActionReceiver::class.java).apply { action = BudgetCycleActionReceiver.ACTION_START_BUDGET; putExtra(BudgetCycleActionReceiver.EXTRA_EXPENSE_ID, expenseId) }
            ...
            val notification = NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(android.R.drawable.ic_dialog_info)
                .setContentTitle("Income received").setContentText("${amountMinor.formatAsCurrency()} received. Use it to start the next budget cycle?")
    ...
    fun notifySmsImportMissed() {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(android.R.drawable.ic_dialog_alert).setContentTitle("Possible transaction missed").setContentText("A bank SMS was received but couldn't be added automatically.").setStyle(NotificationCompat.BigTextStyle().bigText("A bank transaction SMS was received, but Expense Tracker couldn't process it automatically. Tap to add the transaction manually.")).setPriority(NotificationCompat.PRIORITY_HIGH).setContentIntent(pendingIntent).setAutoCancel(true).build()
    ...
    private fun notifyTransactionAdded(title: String, body: String, summary: String, contentIntent: PendingIntent?) {
        if (!hasNotificationPermission()) return
        val builder = NotificationCompat.Builder(context, CHANNEL_ID).setSmallIcon(android.R.drawable.ic_dialog_info).setContentTitle(title).setContentText(summary).setStyle(NotificationCompat.BigTextStyle().bigText(body)).setPriority(NotificationCompat.PRIORITY_DEFAULT).setAutoCancel(true)
    ...
    private fun createEditExpensePendingIntent(expenseId: Long): PendingIntent = PendingIntent.getActivity(context, expenseId.hashCode(), Intent(context, MainActivity::class.java).apply { action = MainActivity.ACTION_OPEN_EDIT_EXPENSE; flags = Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP; putExtra(MainActivity.EXTRA_EXPENSE_ID, expenseId) }, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
